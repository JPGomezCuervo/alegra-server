...
import { createSeller, getAllSellers  } from "../controllers/sellersControllers.js";
...
export const postSellers = async (req: Request, res: Response) => {
    ...
    try {
        ...
        return res.status(200).json(`La cuenta fue creada con éxito: ¡${respose.name} a vender!`);
    } catch (error) {
        if (error instanceof Error){
            res.status(400).json(error.message);
        } else {
